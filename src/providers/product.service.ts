...
export class ProductService {
  ...
  async addProduct(product: Product, captureData: string) {
    ...
    let nextProductKey = this.database
      .list(`/products/${product.shopRef}`)
      .push({}).key;
    if (captureData) {
      let imgSrc = `images/shops/${
        product.shopRef
      }/products/${nextProductKey}/${Date.now()}.jpg`;
      this.imageService
        .uploadImage(imgSrc, captureData)
        .then((snapshot: any) => {
          product.image = snapshot.downloadURL;
          this.database
            .object(`/products/${product.shopRef}/${nextProductKey}`)
            .set(product);
        });
    } else {
      this.database
  ...
  updateProduct(
    ...
    this.database
      .list(`/products/${shopRef}`)
      .update(productRef, productProperties);
  }

  deleteProduct(product: Product) {
    // delete data
    this.database
      .object(`products/${product.shopRef}/${product.$key}`)
      .remove();
    // delete image
  ...
  getProducts(query = {} as any): any {
    ...
    return list.map(array => array.reverse()) as FirebaseListObservable<
      Product[]
    >;
  }
